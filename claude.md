...
1. **api_client.py** - ThetaData API wrapper
   ...
   - `get_dates(symbol, expiration)` → list of (symbol, expiration, trade_date)
     - Raises exceptions (no try-except) - handled by download script
   - `get_greeks_history(symbol, expiration, trade_date)` → raw CSV string
...
2. **database.py** - SQLite database manager
   ...
   - Helper methods:
     ...
     - `claim_next_row(max_retries)` - Atomically claim pending row (ORDER BY expiration DESC, trade_date DESC)
...
available_dates: (symbol, expiration, trade_date) - UNIQUE, FK to expirations
...
- `/v3/option/history/greeks/all?symbol=SPX&expiration=20250117&date=20250103&interval=5s` (Note: API parameter is 'date', but our code uses 'trade_date')
...
- **Download order**: Latest expirations first (ORDER BY expiration DESC, trade_date DESC)
...
- **Retry failed Greeks downloads**: Use SQL to reset failed rows to pending
  ...
  - Reset specific row: `UPDATE available_dates SET status='pending', retry_count=0 WHERE symbol='...' AND expiration='...' AND trade_date='...'`
...
## Lessons Learned
- **Table naming**: Avoid SQL reserved keywords (changed `dates` → `available_dates`, `date` → `trade_date`)
